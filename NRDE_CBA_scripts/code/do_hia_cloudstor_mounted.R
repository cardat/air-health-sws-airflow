...
tab_summary_ste
dir(OUTPUT_FOLDER, pattern = "tab")
output_csv <- file.path(OUTPUT_FOLDER, "tab_summary_ste.csv")
write.csv(tab_summary_ste, output_csv, row.names = F)
...
dir(OUTPUT_FOLDER, pattern = "tab")
output_csv <- file.path(OUTPUT_FOLDER, "tab_summary_gcc.csv")
write.csv(tab_summary_gcc[zone %in% c("G", "A") & date_year == 2015][order(pollutant, state)]
          , output_csv, row.names = F)
