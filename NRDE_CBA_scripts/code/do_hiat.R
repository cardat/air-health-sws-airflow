...
# i = 1
  ...
  for(yy in yy_todo){
    # yy = yy_todo[1]
    indata_f <- tbl_hia_baseline[SA2_MAIN16==sa2_i & date_year == yy]
  ...
  if(i == 1){
    ...
    out_master <- rbind(out_master, out1)
  }
  # end loop by sa2
}
...
an_sa2_year <- out_master[,.(var = sum(an)),by = .(SA2_MAIN16, date_year, est_type)]
an_sa2_year <- dcast(an_sa2_year, SA2_MAIN16 + date_year ~ est_type, value.var = "var")
an_sa2_year
nrow(an_sa2_year[date_year == 2016])

an_nat_year <- an_sa2_year[,.(est = sum(est, na.rm=TRUE), lci = sum(lci, na.rm=TRUE), uci = sum(uci, na.rm=TRUE)), by = .(date_year)]

yll_sa2_year <- out_master[,.(var = sum(yll)),by = .(SA2_MAIN16, date_year, est_type)]
yll_sa2_year <- dcast(yll_sa2_year, SA2_MAIN16 + date_year ~ est_type, value.var = "var")
...
yll_nat_year <- yll_sa2_year[,.(est = sum(est, na.rm=TRUE), lci = sum(lci, na.rm=TRUE), uci = sum(uci, na.rm=TRUE)), by = .(date_year)]

le_sa2_year <- out_master[difference.age == "0 - 4", .(SA2_MAIN16, difference.ex_diff, est_type, date_year)]
le_sa2_year <- dcast(le_sa2_year, SA2_MAIN16 + date_year ~ est_type, value.var = "difference.ex_diff")

# SA2 pop for life expectancy weighted average
pop_SA2_year <- out_master[,.(population = sum(population)), by = .(date_year, SA2_MAIN16)]

le_sa2_year <- merge(le_sa2_year, pop_SA2_year, by = c("SA2_MAIN16", "date_year"), all.x = T)
...
#### overall ####
le_nat_year
le_nat_av_est = mean(le_nat_year$est)
le_nat_av_lci = mean(le_nat_year$lci)
le_nat_av_uci = mean(le_nat_year$uci)
...
an_state_year <- an_sa2_year[,.(est = sum(est, na.rm=TRUE), lci = sum(lci, na.rm=TRUE), uci = sum(uci, na.rm=TRUE)), by = .(state, date_year)]
an_state_year

an_state_av_est = an_state_year[,.(est = mean(est), lci = mean(lci), uci = mean(uci)), by = .(state)]
...
yll_state_year <- yll_sa2_year[,.(est = sum(est, na.rm=TRUE), lci = sum(lci, na.rm=TRUE), uci = sum(uci, na.rm=TRUE)), by = .(state, date_year)]
yll_state_year

yll_state_av_est = yll_state_year[,.(est = mean(est), lci = mean(lci), uci = mean(uci)), by = .(state)]

names(an_state_av_est)[2:4] <- paste0("an_", names(an_state_av_est)[2:4])
tab_summary_ste <- merge(an_state_av_est, yll_state_av_est)[rev(order(an_est))]
...
an_state_av_est <- an_state_year[,.(est = mean(est), lci = mean(lci), uci = mean(uci)), by = .(state, date_year)]
...
yll_state_av_est = yll_state_year[,.(est = mean(est), lci = mean(lci), uci = mean(uci)), by = .(state, date_year)]

names(an_state_av_est)[3:5] <- paste0("an_", names(an_state_av_est)[3:5])
tab_summary_ste <- merge(an_state_av_est, yll_state_av_est)[rev(order(an_est))]
tab_summary_ste[8,"state"] <- "National"
...
tab_summary_ste$zone <- substr(tab_summary_ste$state, 2,2)

tab_summary_ste[zone %in% c("G", "A") & date_year == 2015][order(state)]
dir("figures_and_tables", pattern = "tab")
write.csv(tab_summary_ste[zone %in% c("G", "A") & date_year == 2015][order(state)]
