...
error_cond <- is.null(opt$output_folder) | is.null(opt$root_input_dir)
if (error_cond) {
...
# check data and output folders if they exists
if (!dir.exists(DATA_ROOT_FOLDER)) {
  stop(cat('Provided data folder ', DATA_ROOT_FOLDER, ' do not exists'), call. = FALSE)
}
...
# Setup directories
source(file.path(BASE_FOLDER, "code/dir_setup_cloustor_mounted.R"))
...
# Run HIA
source(file.path(BASE_FOLDER, "code/do_hia_cloudstor_mounted.R"))
