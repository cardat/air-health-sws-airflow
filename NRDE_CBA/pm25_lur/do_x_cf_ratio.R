...
if(do_x_cf_ratio){
  ...
  tbl_x_mb_adj <- tbl_x_mb_adj[, value := value_adj]
...
tbl_x_cf_ste <- tbl_x_mb[,min(valueV2, na.rm = T), by = .(STE_CODE11, date_year)]
...
# summary(x2$value)
# plot(density(x2$value, na.rm = T))
# lines(density(x2$valueV2, na.rm = T), col = "blue")
...
saveRDS(tbl_x_cf_ste, "working_temporary/pm25_lur/tbl_x_cf_ste.rds")
